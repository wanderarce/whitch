...
import {
    addFreeAdsComment, deslikeFreeAds, searchFreeAds, likeFreeAds,
...
const ShowOneFreeAds: React.FC = () => {
    const navigation = useNavigation();
    const [freeAds, setFreeAds] = useState([]);
    const [showComments, setShowComments] = useState([]);
    const [commentsLocked, setCommentsLocked] = useState([]);
    const [profile, setProfile] = useState({});
    const [title, setTitle] = useState('MEUS CLASSIFICADOS');
    const [showMore,setShowMore] = useState(true);
    const [ofsset, setOffset] = useState();
    ...
    const loadFreeAds = async () => {
        ...
        const freeAdsAsString = await AsyncStorage.getItem('showFreeAds');
        const freeAdsShow = await JSON.parse(freeAdsAsString);
        const title = await AsyncStorage.getItem('showFreeAdsTitle');
        setTitle(title);

        const newAds = [freeAdsShow].map((ads) => loadInitialShowImage(ads));
        setFreeAds(newAds);
    };
